<script>
    ...
    import { preFormInput, preFormErrors } from '../../store';

    $preFormInput.startTime = currentDateTime
        .startOf('hour')
        .format('HH:mm');
    ...
    // Validation
    $: if (
        !$preFormInput.startTime.match(startTimeRegex) &&
        $preFormInput.startTime.length > 0
    ) {
