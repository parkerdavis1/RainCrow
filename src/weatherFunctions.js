...
export function parseWeather(times, weatherResults) {
    ...
    parsedWeather.temperature.f =
        dataRange(Math.round(startTemp), Math.round(endTemp)) + '°F';
    parsedWeather.temperature.c =
        dataRange(
            Math.round(convertToCelsius(startTemp)),
            Math.round(convertToCelsius(endTemp))
        ) + '°C';
    ...
    parsedWeather.windspeed.mph =
        dataRange(
            Math.round(windspeed.start.avg),
            Math.round(windspeed.end.avg)
        ) + 'mph';
    parsedWeather.windspeed.ms =
        dataRange(
            Math.round(mphToMs(windspeed.start.avg)),
            Math.round(mphToMs(windspeed.end.avg))
        ) + 'm/s';
    parsedWeather.windspeed.kmh =
        dataRange(
            Math.round(mphToKmh(windspeed.start.avg)),
            Math.round(mphToKmh(windspeed.end.avg))
        ) + 'km/h';
    ...
    parsedWeather.windspeed.description = capitalizeFirst(
        dataRange(
            mphToDescription(windspeed.start.avg),
            mphToDescription(windspeed.end.avg)
        )
    );
    if (windspeed.start.gusts || windspeed.end.gusts) {
        ...
        let windspeedFilteredArr = windspeedArr.filter(
            (entry) => entry || entry === 0
        );
        ...
        parsedWeather.windspeed.beaufort = dataRange(
            ...
            Math.round(
                mphToBeaufort(sortedWindspeedArr[sortedWindspeedArr.length - 1])
            )
        );
        parsedWeather.windspeed.description = capitalizeFirst(
            dataRange(
                ...
                mphToDescription(
                    sortedWindspeedArr[sortedWindspeedArr.length - 1]
                )
            )
        );
    }
    ...
    parsedWeather.timezone = `Timezone: ${
        weatherResults.start.timezone
    } (${timezoneParse(times.offset)})`;
...
export async function getWeather(times, locationObj, weatherResults, language) {
    console.log(
        'Start time weather query for ' +
            times.start.utcTime.local().format('YYYY-MM-DD h:mma Z')
    );
    ...
    if (
        times.end.utcTime &&
        times.end.utcTime.format('X') != times.start.utcTime.format('X')
    ) {
        console.log(
            'End time weather query for ' +
                times.end.utcTime.local().format('YYYY-MM-DD h:mma Z')
        );
...
export function convertToUnixTime(times) {
    times.start.utcTime = dayjs(times.start.localTime)
        .utc(true)
        .subtract(times.offset, 'seconds');
    if (times.end.localTime) {
        times.end.utcTime = dayjs(times.end.localTime)
            .utc(true)
            .subtract(times.offset, 'seconds');
    }
    return times;
...
export async function getTimezoneOffset(times, locationObj) {
    ...
    try {
        ...
        const timezoneQuery = await queryOpenWeather(
            unixTime,
            locationObj.lat,
            locationObj.lon
        );
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (jsonResponse.durationHrs) {
                ...
                times.end.localTime = dayjs(
                    checklistInfo.endTime,
                    'YYYY-MM-DD HH:mm'
                );
            }
            ...
            const coordUrl =
                'https://api.ebird.org/v2/ref/region/info/' +
                checklistInfo.locationId;

            const response2 = await fetch(coordUrl, requestOptions);
            if (response2.ok) {
                const jsonResponse2 = await response2.json();
                checklistInfo.lon =
                    (jsonResponse2.bounds.maxX + jsonResponse2.bounds.minX) / 2; //get average point in middle of bounds
                checklistInfo.lat =
                    (jsonResponse2.bounds.maxY + jsonResponse2.bounds.minY) / 2;
